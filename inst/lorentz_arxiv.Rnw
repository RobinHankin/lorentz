...
\title{Special relativity in R: introducing the {\tt lorentz} package and a systematic search for a distributive law for three-velocities}
...
physics.  The package includes functionality for Lorentz transforms
and Einsteinian three-velocity addition, which is noncommutative and
nonassociative.  I use the package to systematically sweep a large
space of potential distributive laws, without success.  The package is
...
The {\tt lorentz} package provides {\tt R}-centric functionality
...
three-velocities, that is, finding an expression for $r\odot(u\oplus
v)$ where $u$ and $v$ are three-velocities and $r$ is a real number.
We define a large number of possible combinations of $u$, $v$, and $r$
using {\tt lorentz} package idiom.  These are calculated and the
results compared with the exact value.  If there is an exact
expression, this will have zero discrepancy.
...
The basic idea is that $r\odot(u\oplus v) = r\odot u+r\odot v + X$
...
three-velocities; {\tt ru} and {\tt su} represent scalar mutliples of
$u$ $r\odot u$ and $s\odot u$.  Note the variety of orders
(three-velocity addition is not commutative), different bracketing
(three-velocity addition is not associative), and use of $r$ or $1/r$
in different places in the formula.
