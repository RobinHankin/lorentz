...
because $c\neq 1$.  Note how the transform is essentially the Galilean
...
The boost matrix is instructive:

<<boostinfc>>=
boost(u)
@ 

With an infinite speed of light, even ``large'' speeds have zero
...
Function \code{rboost()} will return a random Lorentz transform
matrix, which is in general a combination of a pure Lorentz boost and
an orthogonal rotation.  With an infinite speed of light, it requires
a speed:
...
It is easy to define a disco ball:
...
Then we define a null geodesic parallel to the x-axis, corresponding
to photons, and reflect them in the disco ball:
...
we might ask what percentage of photons are reflected towards the source:
...
Note that a spinning disco ball would give the same (instantaneous)
results.
