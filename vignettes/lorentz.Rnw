...
Lorentz transforms operate on four-vectors such as four-velocity or
four-potential and are usually operationalised as multiplication by a
$4\times 4$ matrix.  A Lorentz transform takes the components of
...
(trivial function \code{flob()} sets the dimnames appropriately).  The
boost matrix is not symmetrical, even though it is a pure boost,
