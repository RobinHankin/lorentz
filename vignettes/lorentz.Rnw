...
The package makes computations easy.  Suppose $v=0.6c$ in the
$x$-direction.
...
Observe that it is possible to transform an arbitary four-vector:
...
\section[Units in which $c=1$]{Units in which $\mathbf c\neq 1$}  
...
{\em row} of ${\mathbf V}$ is four-velocity.  Because a vector is
...
The package includes functionality for dealing with the stress-energy
tensor (sometimes the energy-momentum tensor):
...
The stress-energy tensor is usually considered with two upstairs
(contravariant) indices, as in~$T^{\alpha\beta}$; it may be
transformed using the \code{transform_uu()} function: package:

<<>>=
B <- boost(as.3vel(c(0.4,0.2,0.5)))
transform_uu(dust(1),B)
@ 

See how in this reference frame, the stress-energy tensor has
components corresponding to nonzero pressure and momentum transfer.
...
<<>>=
photongas(1)
transform_dd(photongas(1),B)
@

and as a consistency check we can verify that this is the same as
transforming the SET with upstairs indices:

<<>>=
lower(transform_uu(lower(photongas(1)),lower(B))) %>% flob()
@ 
...
Successive boots are represented as matrix multiplication.  Again the
\pkg{magrittr} package can be used for more readable idiom.
 
<<>>=
B1 <- boost(as.3vel(c(0.5,-0.4,0.6)))
B2 <- boost(as.3vel(c(0.1,-0.1,0.3)))
perfectfluid(4,1)
perfectfluid(4,1) %>% transform_uu(B1) %>% transform_uu(B2)
perfectfluid(4,1) %>% transform_uu(B2 %*% B1)  # should match
@                                                            
...
associative~\citep{ungar2006}; this observation was the original
motivation for the package.  \citeauthor{ungar2006} shows that the
