...
this can be changed).  Three-velocity $u$ may be expressed as a
four-velocity:
...
relativity.  I will discuss passive transforms first, then active
transforms.
...
speed $v$ past me; and we are wondering what various vectors look like
to one another.
...
see a four-vector $\vvec{1}{0}$, and he sees $\vvec{\gamma}{-\gamma
  v}$.  So I see the object at rest, and he sees it as moving at speed
$-v$, that is, to the left (it moves to the left because he is moving
to the right relative to me).
...
shifting: he is moving to the right and bashes into a photon moving to
the left.
...
This is why it makes sense to use notation of $x'$ and $x$ with a
passive Lorentz transformation: the prime denotes measurements made
using coordinates that are defined with respect to the boosted system,
...
This is the first two elements of a displacement four-vector.  It is
...
sol(10)
options(digits=3)
u <- runif(7,3,4)  # speed in the x-direction [c=10]
U <- cbind(gam(u),gam(u)*u,0,0)
(U <- as.4vel(U))
@ 
...
Successive boosts are effected by matrix multiplication:
...
With \code{tcrossprod()}:

<<>>=
tcrossprod(U, B2 %*% B1)
@ 
  
Again magrittr provides arguably more readable idiom:
...
Note carefully that the following natural idiom is {\bf INCORRECT}:
...
<<>>=
sol(1)
(A <- as.photon(as.3vel(cbind(0.9,runif(5)/100,runif(5)/100))))
@ 

above, $A$ is a vector of four-momentum of 10 photons following a null
path.  They are all moving approximately parallel to the x-axis.  We
can check that this is indeed a null vector:
                                                 
<<>>=
inner4(A)
@                                                  

showing that the vectors are indeed null.  We are going to reflect the
photons in an oblique mirror which is itself moving at half the speed
of light along the x-axis.  The first step is to define the mirror:
...
Above, see how the photons have lost energy because of a redshift.
Next, reflect the photons in the mirror (which is at rest):
...
Consider the following R idiom, in which we take a bunch of photons
which, in a certain reference frame are all moving (almost) parallel
to the x-axis.  Then we reflect the photons from a mirror which is
moving with a composition of pure boosts, and examine the reflected
light in their original reference frame:


<<>>=
sol(1)
Astart <- as.photon(as.3vel(cbind(0.9,runif(5)/1e4,runif(5)/1e4)))
A <- Astart
