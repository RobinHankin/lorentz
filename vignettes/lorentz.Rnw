...
transfer, and the $[0,0]$ component is greater, at 2.78, than its rest
value of 1.  If one wants to consider the stress-energy tensor with
downstairs indices (here we will use a photon gas), we need to use
\code{transform_dd()}:
...
is the same as transforming the SET with upstairs indices:

<<>>=
lower(transform_uu(lower(pg),lower(B))) %>% flob()
@ 
