...
$y$- annd $z$- components being trivial.  A typical physical
...
my friend sees the photon with energy $\gamma(1-v)=\sqrt{1-v}<1$: the
...
because $c\neq 1$.  Note how the transform is essentially the
...
Thus four-vector \code{disp} is the four-displacement corresponding to
moving 1 meter along the x-axis and waiting for one second.  To
...
$3\times 10^{-9}$ seconds and the unit of distance remains the meter.
Alternatively, we might decide to keep the unit of time equal to one
second, and use a unit of distance equal to 299792458 meters which
...
the same coordinate transformation but expressed using different
speeds of light, and verify that their orthogonal component agrees:
...
D <- dust(1)  # "dust" is the simplest nontrivial SET
D             # dust has only one nonzero component
@ 
...
the reflection is performed using a sequence of transforms.
First, transform the
...
Here I provide numercial verification of equations~\ref{noncom}
