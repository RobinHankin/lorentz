...
boost matrices and their transposes we will use $c=10$.
...
Anyway, in this context we really ought to use \code{tcrossprod()}:
...
The above transforms were passive: we have some four-vectors and are
now looking at those four-vectors from the perspective of my friend
Baz, who is moving in the positive x direction at 60\% of the speed of
light (remember that $c=10$).  See how the x-component of the
transformed four-velocity is negative, because from Baz's perspective
the four velocities are moving backwards.
...
First we will do some passive boosts.
...
Again magrittr provides arguable more readable idiom:
