...
the four-velocity of an object at rest).  What this means is that {\bf
  he} sees a four-vector $\vvec{1}{0}$, and {\bf I} see
$\vvec{\gamma}{\gamma v}$.  So he sees the object at rest, and I see
it as moving at speed $v$ to the right.
...
So if we consider unit energy and keep $c=1$ we get $p=\vvec{1}{1}$ in
our one-dimensional world and the Lorentz transform is then

\begin{equation*}
  \twomat{\gamma}{\gamma v}{\gamma v}{\gamma}\vvec{1}{1}=\vvec{\gamma+\gamma v}{\gamma+\gamma v}
  \end{equation*}

So he sees a photon with unit energy, and I see the photon with energy
$\gamma(1+v)=\sqrt{\frac{1+v}{1-v}}>1$: the photon has more energy in
my frame than his because of Doppler blue-shifting.
...
other.  Consider a four-vector that might be a four-velocity, a
four-momentum, etc.  Then different observers will disagree about how
to describe that four-velocity.  Thus one observer might write
$\vvec{1}{0}$ and another would write $\vvec{\gamma}{\gamma v}$.  Such
transforms are commonplace in classical world: If I stand on the North
pole of Earth, then I might describe a locally vertical vector as
$\left(\begin{array}{c}1\\0\\0\end{array}\right)$ but my friend on the
  equator would describe the {\em same} vector as
  $\left(\begin{array}{c}0\\1\\0\end{array}\right)$.  Noone is right
    or wrong here, we are just using different coordinate systems.

This is why it makes sense to use notation of $x'$ and $x$ with a
passive Lorentz transformation: the prime denotes coordinates that are
defined with respect to the boosted coordinate system.
...
Again, we have me at rest and my friend in a spaceship, but this time
he observes a four-vector with certain components, and we are
wondering what I would observe.  This is an {\em active} transform
because, for example, he would see a four-velocity of $\vvec{1}{0}$,
and I say to myself: ``My friend sees $\vvec{1}{0}$, and I know what
that means.  He sees an object at rest.  What does that vector look
like, to me?".  This is an {\em active} transform.  We can
vector $\vvec{1}{0}$ and
