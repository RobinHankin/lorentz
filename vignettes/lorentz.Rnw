...
Then, as a consistency check, we will verify that transforming to a
frame in which the fluid is at rest will result in a stress-energy
tensor that matches \code{pf_rest}:
