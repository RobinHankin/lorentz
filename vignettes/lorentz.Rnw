...
The gamma correction term is negligible as far as the default print
method is concerned:
...
But we can display more accurately by subtracting one:
...
or, more accurately by using the \code{gamm1()} function:
