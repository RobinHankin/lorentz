...
<<>>=
sol(1)    # revert to default speed of light: c=1
u <- as.3vel(c(0.3,-0.4,0.8))
u
@ 

The three-velocity $u$ may be expressed as a four-velocity using the
\code{as.4vel()} function:

<<>>=
as.4vel(u)
@ 

Function \code{boost()} gives the corresponding coordinate
transformation when given either a three-velocity or a four-velocity:
...
As we saw above, transforming an arbitrary four-vector from the rest
frame to the moving one has straightforward R idiom:
...
expected).  However, things are different if we wish to consider many
four-vectors in one R object.  A vector ${\mathbf V}$ of
four-velocities is a matrix: each {\em row} of ${\mathbf V}$ is
four-velocity.  Because a vector is treated (almost) as a one-column
matrix in R, and the four-velocities are rows, we need to take a
transpose in some sense.  
