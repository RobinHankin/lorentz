...
relativity.  I will talke about passive transforms first, then active
transforms.
...
the four-velocity of an object at rest).  What this means is that {\bf
  I} sees a four-vector $\vvec{1}{0}$, and {\bf he} sees
$\vvec{\gamma}{-\gamma v}$.  So I see the object at rest, and he sees
it as moving at speed $-v$ to the right (it moves to the left because
he is moving to the right relative to me).
...
defined with respect to the boosted coordinate system and we see
things like
...
he calculates their four-velocity.  We both agree on this
four-velocity as our reference frames are identical: we have no
relative velocity.

Now his spaceship acquires a constant velocity, leaving me outside the
spaceship, stationary.  My friend continues to fire bullets out of his
gun and sees that their four-velocity, as viewed in his spaceship
coordinates, is the same as when we were together.
...
with respect to me, and one in the spaceship when moving.  Suppose,
for example, he sees the bullets at rest in his spaceship; they have a
four-velocity of $\vvec{1}{0}$, and I say to myself: ``My friend sees
$\vvec{1}{0}$, and I know what that means.  He sees an object at rest.
What are the bullets' four velocities in my reference frame?".  This
is an {\em active} transform:  
