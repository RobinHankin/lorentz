...
classical equivalent, the Galilean transforms.  Lorentz transforms
operate on four-vectors such as four-velocity or four-potential and
are usually operationalised as multiplication by a $4\times 4$ matrix.
A Lorentz transform takes the components of four-vector as observed in
one coordinate system and returns the components observed in another
system which is moving at constant velocity with respect to the first.
...
Then we can define \code{f()} to be themap~$\bx\mapsto\gyr{u}{v}\bx$.
In R:
...
note how the transformation is essentially the Galilean result, which
would be

<<>>=
gal <- cbind(c(1,-100*(1:3)),rbind(0,diag(3)))
rownames(gal) <- c("t","x","y","z")
colnames(gal) <- c("t","x","y","z")
gal
@ 

The boost matrix is not symmetrical, even though it is a pure boost,
because $c\neq 1$.  We can compose boosts as expected:
...
<<>>=
light_start %>% tcrossprod(B) %>% reflect(m) %>% tcrossprod(solve(B))
@ 
