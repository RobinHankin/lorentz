...
\begin{eqnarray}
\gyr{u}{v}\bx\cdot\gyr{u}{v}\by &=& \bx\cdot\by\\
\gyr{u}{v}\left(\bx\oplus\by\right) &=& \gyr{u}{v}\bx\oplus\gyr{u}{v}\by\\
\left(\gyr{u}{v}\right)^{-1} &=& \left(\gyr{v}{u}\right)\\
\bu\oplus\bv &=& \gyr{u}{v}\left(\bv\oplus\bu\right)\\
\bu\oplus\left(\bv\oplus\bw\right) &=&\left(\bu\oplus\bv\right)\oplus\gyr{u}{v}\bw\\
\left(\bu\oplus\bv\right)\oplus\bw &=&\bu\oplus\left(\bv\oplus\gyr{v}{u}\bw\right)
\end{eqnarray}
...
x <- as.3vel(c(-0.2,-0.3,+0.5))
u
@ 

Here we have three-vectors \code{u}, \code{v} and \code{x}. 
We can see that \code{u} and \code{v} do not commute:
...
and also verify Equation ASDF:
<<>>=
u+v
gyr(u,v,v+u)
@ 

We may also use the subtraction idiom:

<<subid>>=
(u+v) - gyr(u,v,v+u)
@ 

showing agreement to within numerical error.

It is also possible to use the functional idiom:
